 ...
 *   entirety and paste it here, changing the prefix from theme_ to orgrightA_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: orgrightA_breadcrumb()
 *
 *   where orgrightA is the name of your sub-theme. For example, the
 ...
 */
function orgrightA_theme(&$existing, $type, $theme, $path) {
...
/***
function orgrightA_image($path, $alt = '', $title = '', $attributes = NULL, $getsize = TRUE) {
...
function orgrightA_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_node_form');
  ...
  case 'member_node_form':
    $panels = orgrightA_member_node_form($form); break;
  case 'subscription_node_form':
  case 'membership_subscription_wizard':
    $panels = orgrightA_subscription_node_form($form); break;
  case 'subpayment_node_form':
    $panels = orgrightA_subpayment_node_form($form); break;
  case 'committee_node_form':
    $panels = orgrightA_committee_node_form($form); break;
  case 'role_node_form':
    $panels = orgrightA_role_node_form($form); break;
  case 'appointment_node_form':
    $panels = orgrightA_appointment_node_form($form); break;
  case 'assignment_node_form':
    $panels = orgrightA_assignment_node_form($form); break;
  case 'meeting_node_form':
    $panels = orgrightA_meeting_node_form($form); break;
  case 'agenda_item_node_form':
    $panels = orgrightA_agenda_item_node_form($form); break;
  case 'mtg_item_node_form':
    $panels = orgrightA_mtg_item_node_form($form); break;
  case 'project_node_form':
    $panels = orgrightA_project_node_form($form); break;
  case 'projactivity_node_form':
    $panels = orgrightA_projactivity_node_form($form); break;
  case 'projreport_node_form':
    $panels = orgrightA_projreport_node_form($form); break;
  case 'drawer_node_form':
    $panels = orgrightA_drawer_node_form($form); break;
  case 'folder_node_form':
    $panels = orgrightA_folder_node_form($form); break;
  case 'document_node_form':
    $panels = orgrightA_document_node_form($form); break;
  case 'meeting_template_use_form':
    $panels = orgrightA_meeting_node_form($form);
    if ($form['step']['#value'] == 2) { $panels['bottom'] .= drupal_render($form['template']); }
    else { $panels['bottom'] = ''; }
    break;
  default:
  }
  ...
  if (!empty($admin)) { $output .= '<div class="admin">' . $admin . '</div>'; }
  $output .= $buttons;
 ...
 */
function orgrightA_member_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_member_node_form');
  ...
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'member');
...
function orgrightA_subscription_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_subscription_node_form');
  // add fields to left upper panel
  if (isset($form['member'])) { $panels['lupper'] .= drupal_render($form['member']); }
  if (isset($form['category'])) { $panels['lupper'] .= drupal_render($form['category']); }
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'subscription');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_subpayment_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_subpayment_node_form');
  // add fields to left upper panel
  $panels['lupper'] .= drupal_render($form['member']);
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'subpayment');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_committee_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_committee_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'committee');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_role_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_role_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'role');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_appointment_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_appointment_node_form');
  // add fields to left upper panel
  $panels['lupper'] .= drupal_render($form['member']);
  $panels['lupper'] .= drupal_render($form['committee']);
  // add fields to right upper panel
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'appointment');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_assignment_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_assignment_node_form');
  // add fields to left upper panel
  $panels['lupper'] .= drupal_render($form['member']);
  $panels['lupper'] .= drupal_render($form['committee']);
  $panels['lupper'] .= drupal_render($form['role']);
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'assignment');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_meeting_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_meeting_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'meeting');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_agenda_item_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_agenda_item_node_form');
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'agenda_item');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_mtg_item_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_mtg_item_node_form');
  // add fields to middle panel
  $panels['middle'] .= drupal_render($form['meeting']);
  // add fields to left lower panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'mtg_item');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_project_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_project_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'project');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_projactivity_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_projactivity_node_form');
  // add fields to left upper panel
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'projactivity');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_projreport_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_projreport_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'projreport');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_drawer_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_drawer_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'drawer');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_folder_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_folder_node_form');
  // add fields to left upper panel
  ...
  $panels['bottom'] .= drupal_render($form['body']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'folder');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_document_node_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_document_node_form');
  // add fields to left upper panel
  ...
  // add fields to bottom panel
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  $panels['bottom'] .= orgrightA_render_custom_fields($form, 'document');
  $panels['bottom'] .= drupal_render($form['notes']);
  $panels['bottom'] .= '<div class="member rule-off"></div>';
  // finished this form
  return $panels;
...
function orgrightA_membership_import_members_form(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_membership_import_members_form');
...
function orgrightA_membership_subscription_wizard(&$form) {
  //orgright_debug_msg('portal','Fn: orgrightA_membership_subscription_wizard');
  ...
  if ($form['step']['#value'] == 1) {
    return orgrightA_node_form($form);
  }
...
function orgrightA_meeting_template_use_form(&$form) {
 ...
 */
function orgrightA_render_custom_fields(&$form, $type) {
  ...
  if (variable_get('orgright_custom_fields_'.$type, 0)) {
    ...
    if ($output) { $output .= '<div class="'.$type.' rule-off"></div>'; }
  }
  return $output;
}
...
function orgrightA_preprocess(&$vars, $hook) {
 ...
 */
function orgrightA_preprocess_page(&$vars, $hook) {
  global $user;
  ...
  // Align the fotter message to the left, and add a theme designer link
  ...
  $vars['footer_message'] .= l(t('Theme by Mainspring'),'http://www.mainspring.co.nz', $options);
 ...
 */
function orgrightA_preprocess_node(&$vars, $hook) {
  // Optionally, run node-type-specific preprocess functions, like
  // orgrightA_preprocess_node_page() or orgrightA_preprocess_node_story().
  ...
  switch ($type) {
  case 'member':
    $vars['content'] = orgrightA_preprocess_node_member($node);
    return;
  case 'subscription':
    $vars['content'] = orgrightA_preprocess_node_subscription($node);
    return;
  case 'subpayment':
    $vars['content'] = orgrightA_preprocess_node_subpayment($node);
    return;
  case 'committee':
    $vars['content'] = orgrightA_preprocess_node_committee($node);
    return;
  case 'role':
    $vars['content'] = orgrightA_preprocess_node_role($node);
    return;
  case 'appointment':
    $vars['content'] = orgrightA_preprocess_node_appointment($node);
    return;
  case 'assignment':
    $vars['content'] = orgrightA_preprocess_node_assignment($node);
    return;
  case 'meeting':
    $vars['content'] = orgrightA_preprocess_node_meeting($node);
    return;
  case 'agenda_item':
    $vars['content'] = orgrightA_preprocess_node_agenda_item($node);
    return;
  case 'mtg_item':
    $vars['content'] = orgrightA_preprocess_node_mtg_item($node);
    return;
  case 'project':
    $vars['content'] = orgrightA_preprocess_node_project($node);
    return;
  case 'projactivity':
    $vars['content'] = orgrightA_preprocess_node_projactivity($node);
    return;
  case 'projreport':
    $vars['content'] = orgrightA_preprocess_node_projreport($node);
    return;
  case 'drawer':
    $vars['content'] = orgrightA_preprocess_node_drawer($node);
    return;
  case 'folder':
    $vars['content'] = orgrightA_preprocess_node_folder($node);
    return;
  case 'document':
    $vars['content'] = orgrightA_preprocess_node_document($node);
    return;
  default:
  }

}
function orgrightA_preprocess_node_member($node) {
  //orgright_debug_dump('Fn: orgrightA_preprocess_node_member',$node);
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'member');
...
function orgrightA_preprocess_node_subscription($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'subscription');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="subscription rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_subpayment($node) {
  // Subscription type nodes will have 3 panels, upper left & right
  // and a bottom full width region
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'subpayment');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="subpayment rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_committee($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'committee');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="committee rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_role($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'role');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="role rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_appointment($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'appointment');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="appointment rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_assignment($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'assignment');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="assignment rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_meeting($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'meeting');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="meeting rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_agenda_item($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'agenda_item');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="agenda-item rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_mtg_item($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'mtg_item');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="mtg-item rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_project($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= '<div class="project rule-off"></div>';
    $bottom .= orgrightA_content_cck_fields($node, 'project');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="project rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_projactivity($node) {
  // Project type nodes will have 6 panels, upper left & right,
  // a full width middle, then a lower left & right,
  // and a bottom full width region
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'projactivity');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="projactivity rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_projreport($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'projreport');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="report rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_drawer($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'drawer');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="drawer rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_folder($node) {
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'folder');
    $bottom .= $node->content['notes']['#value'];
    $bottom .= '<div class="folder rule-off"></div>';
    if (isset($node->content['terms'])) {
      $bottom .= $node->content['terms'];
...
function orgrightA_preprocess_node_document($node) {
  // Project type nodes will have 6 panels, upper left & right,
  // a full width middle, then a lower left & right,
  // and a bottom full width region
  ...
  if (isset($node->body)) {
    ...
    $bottom .= orgrightA_content_cck_fields($node, 'document');
    $bottom .= $node->content['notes']['#value'];
 ...
 */
function orgrightA_content_cck_fields($node, $type) {
  // Check the database for any custom fields
  $output = '';
  if (variable_get('orgright_custom_fields_'.$type, 0)) {
    $sql = "SELECT field_name FROM {content_node_field_instance} WHERE type_name = '%s' ORDER BY weight";
 ...
 */
/* -- Delete this line if you want to use this function
function orgrightA_preprocess_comment(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function orgrightA_preprocess_block(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
function toplevel_menu_output($tree, $attributes = array('class' => 'links'), $heading = '') {
  $output = '';
  $items = array();
  ...
  foreach ($items as $i => $data) {
    ...
    if ($data['below']) {
      ...
      $output .= theme('menu_item', $link, $data['link']['has_children'], dropdown_menu_output($data['below']), $data['link']['in_active_trail'], $extra_class);
    }
    else {
 ...
 */
function dropdown_menu_output($tree) {
  $output = '';
  $items = array();
 ...
 * as specified
 */
function orgrightA_menu_tree($tree, $attributes = array(), $heading = '') {
  global $user;
 ...
 */
function orgrightA_links($links, $attributes = array('class' => 'links'), $heading = '') {
  global $user;
  // no links for anonymous users
  if (!$user->uid) { return ''; }
...
/*
function orgrightA_links($links, $attributes = array('class' => 'links')) {
  global $user;
  // no links for anonymous users
  if (!$user->uid) { return ''; }
 ...
 */
function orgrightA_orgright_heading($module, $class, $value) {
...
function orgrightA_orgright_item($module, $class, $description, $item, $node_ref = 0) {
...
function orgrightA_orgright_subitem($module, $class, $subitem, $node_ref, $attributes) {
...
function orgrightA_orgright_link($module, $class, $link) {
